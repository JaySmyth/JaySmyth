...
class TransendOrderImport implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            if (isset($reply['RequestStatus']) && $reply['RequestStatus'] == "OK") {
                ...
                Mail::to('it@antrim.ifsgroup.com')->send(new \App\Mail\GenericError('Transend Order Import Failed (' . $this->transportJob->number . ')', $reply['RequestError']));
            }
        } catch (GuzzleException $exc) {

            if ($exc->hasResponse()) {
                Mail::to('it@antrim.ifsgroup.com')->send(new \App\Mail\JobFailed('Transend Order Import GuzzleException (' . $this->transportJob->number . ')', Psr7\str($exc->getResponse())));
            }
        }
    }
    ...
    private function getJobDetails()
    {
        ...
        $details[] = [
            'LineNumber' => 1,
            'Barcode' => 1,
            'Description' => substr($this->transportJob->goods_description, 0, 40),
