...
class CleanAddressTable extends Command
{
    ...
    public function handle()
    {
        ...
        $addresses = \App\Models\Address::whereNotNull('id')->groupBy('company_id', 'name', 'company_name', 'address1')->havingRaw('count(*) > 1')->get();

        $this->info($addresses->count().' duplicate addresses found');

        foreach ($addresses as $duplicate) {
            $duplicateAddress = \App\Models\Address::whereCompanyId($duplicate->company_id)->whereName($duplicate->name)->whereCompanyName($duplicate->company_name)->whereAddress1($duplicate->address1)->orderBy('id', 'ASC')->get();

            $this->line($duplicateAddress->count().' duplicate addresses loaded');

            foreach ($duplicateAddress as $add) {
                $count = \App\Models\Address::whereCompanyId($duplicate->company_id)->whereName($duplicate->name)->whereCompanyName($duplicate->company_name)->whereAddress1($duplicate->address1)->count();
        ...
        $companies = \App\Models\Company::whereDepotId(4)->get();

        foreach ($companies as $company) {
            $deleted = \App\Models\Address::whereCompanyId($company->id)->delete();
