...
class ClearUpTrackingEvents extends Command
{
    ...
    {
        $tracking = \App\Models\Tracking::whereNotNull('shipment_id')->groupBy('shipment_id', 'message', 'status')->havingRaw('count(*) > 1')->orderBy('shipment_id', 'DESC')->limit(20000)->get();

        $this->info($tracking->count().' duplicate tracking events found');

        foreach ($tracking as $duplicate) {
            $duplicateTracking = \App\Models\Tracking::whereShipmentId($duplicate->shipment_id)->whereMessage($duplicate->message)->whereStatus($duplicate->status)->orderBy('id', 'DESC')->get();

            $this->info($duplicateTracking->count().' duplicate events loaded');

            foreach ($duplicateTracking as $event) {
                $count = \App\Models\Tracking::whereShipmentId($duplicate->shipment_id)->whereMessage($duplicate->message)->whereStatus($duplicate->status)->count();
