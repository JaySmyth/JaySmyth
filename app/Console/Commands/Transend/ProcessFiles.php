        ...
        if (($handle = fopen($this->directory . $file, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle, 2000, ',')) !== false) {
                ...
                if ($this->transportJob) {
                    ...
                    // Log the transaction
                    $this->transportJob->log('Transend transaction received', ($this->transendCode) ? $this->transendCode->description : null, $row);
                }
    ...
    protected function handleTransendCode($code, $datetime)
    {
        ...
        if ($this->transendCode) {
            ...
            if ($this->transendCode->notify_department) {
                ...
                $subject = 'Possible issue with ' . verboseCollectionDelivery($this->transportJob->type) . ' ' . $this->transportJob->number . ' / ' . $this->transportJob->reference . ' (' . $this->transportJob->from_company_name . ' > ' . $this->transportJob->to_company_name . ')';
                Mail::to($this->transportJob->department->email)->send(new \App\Mail\GenericError($subject, 'The IFS driver allocated job ' . $this->transportJob->number . ' / ' . $this->transportJob->reference . ' has reported: ' . $this->transendCode->description . '. Please investigate further.'));
            }
